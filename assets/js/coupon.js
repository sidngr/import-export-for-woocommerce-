jQuery(document).ready(function()
{
    ...
    var $export_box = jQuery('.sn-wie-export-page.coupon .export-box');
    var $import_box = jQuery('.sn-wie-import-page.coupon .import-box');
    var $mapping_box = jQuery('.sn-wie-mapping-page.coupon .mapping-box');
    var $schedule_box = jQuery('.sn-wie-schedule-page.coupon .schedule-box');
    var $schedule_profile_box = jQuery('.sn-wie-schedule-profile-page.coupon .schedule-profile-box');
    var $mapping_profile_box = jQuery('.sn-wie-mapping-profile-page.coupon .mapping-profile-box');
    var $log_box = jQuery('.sn-wie-log-page.coupon .log-box');
    ...
    // Toggle Meta row when mapping selected
    ...
    // Select Meta field
    jQuery('.sn-wie-mapping-page.coupon .mapping-box').on('change', '.data-source', function() {
    ...
    jQuery('.sn-wie-mapping-page.coupon .mapping-box .btn-add-column').click(function() {

        var $tr = jQuery('.sn-wie-mapping-page.coupon .mapping-box .mapping-table tbody tr:first-child').clone();
        $tr.find('input').val('');
        $tr.find('select').val('');
        $tr.find('.select2').remove();
        $tr.find('.custom-select').select2();
        jQuery('.sn-wie-mapping-page.coupon .mapping-box .mapping-table tbody').append($tr);
    });
    //------------------------

    // Delete column from mapping
    jQuery('.sn-wie-mapping-page.coupon .mapping-box').on('click', '.btn-delete-column', function() {
    ...
    $export_box.find('.btn-export').click(function() {
        ...
        $export_box.find('.export-form.coupon').ajaxForm({
            beforeSend: function() {
                update_timer();
            ...
            success: function(Response) {
                if(Response['status'] == true) {
                    export_csv_file(Response['data']['export_file_name'], Response['data']['total_requests'], 1, Response['data']['export_columns'], Response['data']['export_meta'], Response['data']['field_separator'], Response['data']['download_filename'], []);
                }
                else {
        ...
        $export_box.find('.export-form.coupon').submit();
    });
    ...
    $import_box.find('.btn-import').click(function() {
        ...
        jQuery('.sn-wie-import-page .import-box .import-form.coupon').ajaxForm({
            beforeSend: function() {
                update_timer();
            ...
            success: function(Response) {
                if(Response['status'] == true) {
                    import_csv_file(Response['data']['file_name'], Response['data']['total_requests'], 0, Response['data']['import_columns'], Response['data']['coupon_search_type'], Response['data']['coupon_operation'], Response['data']['field_separator']);
                }
                else {
                    alert(Response['message']);
                    ...
                    $import_box.find('.import-modal .modal-actions .btn-view-coupons').show();
                }
            },
            error: function() {
        ...
        $import_box.find('.import-form.coupon').submit();
    });
    ...
    function export_csv_file( file_name, total_requests, request_number, export_columns, export_meta, field_separator, download_filename, csv_columns ) {

        jQuery.ajax({
            url: jQuery('.export-form.coupon').attr('action'),
            data: {'action': 'sn_wie_export_coupon_csv_file', 'file_name': file_name, 'total_requests': total_requests, 'request_number': request_number, 'export_columns': export_columns, 'export_meta': export_meta, 'field_separator': field_separator, 'download_filename': download_filename, 'csv_columns': csv_columns},
            type: 'POST',
            success: function(Response) {
                ...
                if(Response['data']['request_number'] <= Response['data']['total_requests']) {
                    ...
                    export_csv_file(Response['data']['file_name'], Response['data']['total_requests'], Response['data']['request_number'], export_columns, export_meta, field_separator, download_filename, Response['data']['csv_columns']);
                }
                else {
                    update_export_status(Response['data']);
                    if(Response['status'] == true) {
                        setTimeout(function() {
                            ...
                            jQuery('body').append('<iframe id="ifrm_download" src="'+sn_wie_admin_url+'admin-post.php?action=sn_wie_download_coupon_export_file&file_name='+file_name+'&download_filename='+download_filename+'" style="display:none;"></iframe>');
    ...
    function import_csv_file( file_name, total_requests, request_number, import_columns, coupon_search_type, coupon_operation, field_separator ) {

        jQuery.ajax({
            url: jQuery('.import-form').attr('action'),
            data: {'action': 'sn_wie_import_coupon_csv_file', 'file_name': file_name, 'total_requests': total_requests, 'request_number': request_number, 'import_columns': import_columns, 'coupon_search_type': coupon_search_type, 'coupon_operation': coupon_operation, 'field_separator': field_separator},
            type: 'POST',
            success: function(Response) {
                ...
                if(Response['data']['request_number'] < Response['data']['total_requests']) {
                    ...
                    import_csv_file( Response['data']['file_name'], Response['data']['total_requests'],Response['data']['request_number'], import_columns, coupon_search_type, coupon_operation, field_separator );
                }
                else {
                    setTimeout(function() {
                        update_import_status(Response['data']);
    ...
    function update_export_status( data ) {
        ...
        export_count += data['coupon_export_count'];
    ...
    function update_import_status( data ) {
        ...
        update_count += data['coupon_update_count'];
        insert_count += data['coupon_insert_count'];
        skip_count += data['coupon_skip_count'];
        fail_count += data['coupon_fail_count'];
        ...
        for(var p=0;p<data['coupon_status'].length;p++) {
            response += '<tr><td class="id">'+data['coupon_status'][p]['id']+'</td><td class="code">'+data['coupon_status'][p]['code']+'</td><td class="message">'+data['coupon_status'][p]['message']+'</td><td class="status">'+(data['coupon_status'][p]['status']==true?'<span class="success">Success</span>':'<span class="error">Error</span>')+'</td></tr>';
        }
