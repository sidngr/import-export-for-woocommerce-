jQuery(document).ready(function()
{
    ...
    var $export_box = jQuery('.sn-wie-export-page.user .export-box');
    var $import_box = jQuery('.sn-wie-import-page.user .import-box');
    var $mapping_box = jQuery('.sn-wie-mapping-page.user .mapping-box');
    var $schedule_box = jQuery('.sn-wie-schedule-page.user .schedule-box');
    var $schedule_profile_box = jQuery('.sn-wie-schedule-profile-page.user .schedule-profile-box');
    var $mapping_profile_box = jQuery('.sn-wie-mapping-profile-page.user .mapping-profile-box');
    var $log_box = jQuery('.sn-wie-log-page.user .log-box');
    ...
    $export_box.find('.btn-export').click(function() {
        ...
        $export_box.find('.export-form.user').ajaxForm({
            beforeSend: function() {
                update_timer();
            ...
            success: function(Response) {
                if(Response['status'] == true) {
                    export_csv_file(Response['data']['export_file_name'], Response['data']['total_requests'], 1, Response['data']['export_columns'], Response['data']['export_user_roles'], Response['data']['export_meta'], Response['data']['field_separator'], Response['data']['download_filename'], []);
                }
                else {
        ...
        $export_box.find('.export-form.user').submit();
    });
    ...
    $import_box.find('.btn-import').click(function() {
        ...
        jQuery('.sn-wie-import-page .import-box .import-form.user').ajaxForm({
            beforeSend: function() {
                update_timer();
            ...
            success: function(Response) {
                if(Response['status'] == true) {
                    import_csv_file(Response['data']['file_name'], Response['data']['total_requests'], 0, Response['data']['import_columns'], Response['data']['user_search_type'], Response['data']['user_operation'], Response['data']['field_separator']);
                }
                else {
                    alert(Response['message']);
                    ...
                    $import_box.find('.import-modal .modal-actions .btn-view-users').show();
                }
            },
            error: function() {
        ...
        $import_box.find('.import-form.user').submit();
    });
    ...
    function export_csv_file(file_name, total_requests, request_number, export_columns, export_user_roles, export_meta, field_separator, download_filename, csv_columns) {

        jQuery.ajax({
            url: jQuery('.export-form.user').attr('action'),
            data: {'action': 'sn_wie_export_user_csv_file', 'file_name': file_name, 'total_requests': total_requests, 'request_number': request_number, 'export_columns': export_columns, 'export_user_roles': export_user_roles, 'export_meta': export_meta, 'field_separator': field_separator, 'download_filename': download_filename, 'csv_columns': csv_columns},
            type: 'POST',
            success: function(Response) {
                ...
                if(Response['data']['request_number'] <= Response['data']['total_requests']) {
                    ...
                    export_csv_file(Response['data']['file_name'], Response['data']['total_requests'], Response['data']['request_number'], export_columns, export_user_roles, export_meta, field_separator, download_filename, Response['data']['csv_columns']);
                }
                else {
                    update_export_status(Response['data']);
                    if(Response['status'] == true) {
                        setTimeout(function() {
                            ...
                            jQuery('body').append('<iframe id="ifrm_download" src="'+sn_wie_admin_url+'admin-post.php?action=sn_wie_download_user_export_file&file_name='+file_name+'&download_filename='+download_filename+'" style="display:none;"></iframe>');
    ...
    function import_csv_file(file_name, total_requests, request_number, import_columns, user_search_type, user_operation, field_separator) {

        jQuery.ajax({
            url: jQuery('.import-form').attr('action'),
            data: {'action': 'sn_wie_import_user_csv_file', 'file_name': file_name, 'total_requests': total_requests, 'request_number': request_number, 'user_search_type': user_search_type, 'user_operation': user_operation, 'field_separator': field_separator},
            type: 'POST',
            success: function(Response) {
                ...
                if(Response['data']['request_number'] < Response['data']['total_requests']) {
                    ...
                    import_csv_file(Response['data']['file_name'], Response['data']['total_requests'], Response['data']['request_number'], import_columns, user_search_type, user_operation, field_separator);
                }
                else {
                    setTimeout(function() {
                        update_import_status(Response['data']);
    ...
    function update_export_status(data) {
        ...
        export_count += data['user_export_count'];
    ...
    function update_import_status(data) {
        ...
        update_count += data['user_update_count'];
        insert_count += data['user_insert_count'];
        skip_count += data['user_skip_count'];
        fail_count += data['user_fail_count'];
        ...
        for(var p=0;p<data['user_status'].length;p++) {
            response += '<tr><td class="id">'+data['user_status'][p]['id']+'</td><td class="username">'+data['user_status'][p]['username']+'</td><td class="message">'+data['user_status'][p]['message']+'</td><td class="status">'+(data['user_status'][p]['status']==true?'<span class="success">Success</span>':'<span class="error">Error</span>')+'</td></tr>';
        }
