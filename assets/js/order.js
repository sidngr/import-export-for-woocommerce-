jQuery(document).ready(function()
{
    ...
    var $export_box = jQuery('.sn-wie-export-page.order .export-box');
    var $import_box = jQuery('.sn-wie-import-page.order .import-box');
    var $mapping_box = jQuery('.sn-wie-mapping-page.order .mapping-box');
    var $schedule_box = jQuery('.sn-wie-schedule-page.order .schedule-box');
    var $schedule_profile_box = jQuery('.sn-wie-schedule-profile-page.order .schedule-profile-box');
    var $mapping_profile_box = jQuery('.sn-wie-mapping-profile-page.order .mapping-profile-box');
    var $log_box = jQuery('.sn-wie-log-page.order .log-box');
    ...
    $mapping_box.find('.btn-add-column').click(function() {
        ...
        var $tr = jQuery('.sn-wie-mapping-page.order .mapping-box .mapping-table tbody tr:first-child').clone();
        $tr.find('input').val('');
        $tr.find('select').val('');
        $tr.find('.select2').remove();
        $tr.find('.custom-select').select2();
        jQuery('.sn-wie-mapping-page.order .mapping-box .mapping-table tbody').append($tr);
    });
    //------------------------
    ...
    $export_box.find('.btn-export').click(function() {
        ...
        $export_box.find('.export-form.order').ajaxForm({
            beforeSend: function() {
                update_timer();
            ...
            success: function(Response) {
                if(Response['status'] == true) {
                    export_csv_file(Response['data']['export_file_name'], Response['data']['total_requests'], 1, Response['data']['export_columns'], Response['data']['export_order_statuses'], Response['data']['export_meta'], Response['data']['field_separator'], Response['data']['download_filename'], []);
                }
                else {
        ...
        $export_box.find('.export-form.order').submit();
    });
    ...
    $import_box.find('.btn-import').click(function() {
        ...
        jQuery('.sn-wie-import-page .import-box .import-form.order').ajaxForm({
            beforeSend: function() {
                update_timer();
            ...
            success: function(Response) {
                if(Response['status'] == true) {
                    import_csv_file(Response['data']['file_name'], Response['data']['total_requests'], 0, Response['data']['import_columns'], Response['data']['order_search_type'], Response['data']['order_operation'], Response['data']['field_separator']);
                }
                else {
                    alert(Response['message']);
                    ...
                    $import_box.find('.import-modal .modal-actions .btn-view-orders').show();
                }
            },
            error: function() {
        ...
        $import_box.find('.import-form.order').submit();
    });
    ...
    function export_csv_file(file_name, total_requests, request_number, export_columns, export_order_statuses, export_meta, field_separator, download_filename, csv_columns) {

        jQuery.ajax({
            url: jQuery('.export-form.order').attr('action'),
            data: {'action': 'sn_wie_export_order_csv_file', 'file_name': file_name, 'total_requests': total_requests, 'request_number': request_number, 'export_columns': export_columns, 'export_order_statuses': export_order_statuses, 'export_meta': export_meta, 'field_separator': field_separator, 'download_filename': download_filename, 'csv_columns': csv_columns},
            type: 'POST',
            success: function(Response) {
                ...
                if(Response['data']['request_number'] <= Response['data']['total_requests']) {
                    ...
                    export_csv_file(Response['data']['file_name'], Response['data']['total_requests'], Response['data']['request_number'], export_columns, export_order_statuses, export_meta, field_separator, download_filename, Response['data']['csv_columns']);
                }
                else {
                    update_export_status(Response['data']);
                    if(Response['status'] == true) {
                        setTimeout(function() {
                            ...
                            jQuery('body').append('<iframe id="ifrm_download" src="'+sn_wie_admin_url+'admin-post.php?action=sn_wie_download_order_export_file&file_name='+file_name+'&download_filename='+download_filename+'" style="display:none;"></iframe>');
    ...
    function import_csv_file(file_name, total_requests, request_number, import_columns, order_search_type, order_operation, field_separator) {

        jQuery.ajax({
            url: jQuery('.import-form').attr('action'),
            data: {'action': 'sn_wie_import_order_csv_file', 'file_name': file_name, 'total_requests': total_requests, 'request_number': request_number, 'import_columns': import_columns, 'order_search_type': order_search_type, 'order_operation': order_operation, 'field_separator': field_separator},
            type: 'POST',
            success: function(Response) {
                ...
                if(Response['data']['request_number'] < Response['data']['total_requests']) {
                    ...
                    import_csv_file(Response['data']['file_name'], Response['data']['total_requests'], Response['data']['request_number'], import_columns, order_search_type, order_operation, field_separator);
                }
                else {
                    setTimeout(function() {
                        update_import_status(Response['data']);
    ...
    function update_export_status(data) {
        ...
        export_count += data['order_export_count'];
    ...
    function update_import_status(data) {
        ...
        update_count += data['order_update_count'];
        insert_count += data['order_insert_count'];
        skip_count += data['order_skip_count'];
        fail_count += data['order_fail_count'];
        ...
        for(var p=0;p<data['order_status'].length;p++) {
            response += '<tr><td class="id">'+data['order_status'][p]['id']+'</td><td class="order_number">'+data['order_status'][p]['order_number']+'</td><td class="message">'+data['order_status'][p]['message']+'</td><td class="status">'+(data['order_status'][p]['status']==true?'<span class="success">Success</span>':'<span class="error">Error</span>')+'</td></tr>';
        }
