jQuery(document).ready(function()
{
    ...
    var $export_box = jQuery('.sn-wie-export-page.product-category .export-box');
    var $import_box = jQuery('.sn-wie-import-page.product-category .import-box');
    var $mapping_box = jQuery('.sn-wie-mapping-page.product-category .mapping-box');
    var $schedule_box = jQuery('.sn-wie-schedule-page.product-category .schedule-box');
    var $schedule_profile_box = jQuery('.sn-wie-schedule-profile-page.product-category .schedule-profile-box');
    var $mapping_profile_box = jQuery('.sn-wie-mapping-profile-page.product-category .mapping-profile-box');
    var $log_box = jQuery('.sn-wie-log-page.product-category .log-box');
    ...
    $export_box.find('.btn-export').click(function() {
        ...
        $export_box.find('.export-form.product-category').ajaxForm({
            beforeSend: function() {
                update_timer();
        ...
        $export_box.find('.export-form.product-category').submit();
    });
    ...
    $import_box.find('.btn-import').click(function() {
        ...
        $import_box.find('.import-form.product-category').ajaxForm({
            beforeSend: function() {
                update_timer();
            ...
            success: function(Response) {
                if(Response['status'] == true) {
                    import_csv_file(Response['data']['file_name'], Response['data']['total_requests'], 0, Response['data']['import_columns'], Response['data']['category_search_type'], Response['data']['category_operation'], Response['data']['field_separator']);
                }
                else {
                    alert(Response['message']);
                    ...
                    $import_box.find('.import-modal .modal-actions .btn-view-product-categories').show();
                }
            },
            error: function() {
        ...
        $import_box.find('.import-form.product-category').submit();
    });
    ...
    function export_csv_file(file_name, total_requests, request_number, export_columns, field_separator, download_filename, csv_columns) {
        ...
        jQuery.ajax({
            url: jQuery('.export-form.product-category').attr('action'),
            data: {'action': 'sn_wie_export_product_category_csv_file', 'file_name': file_name, 'total_requests': total_requests, 'request_number': request_number, 'export_columns': export_columns, 'field_separator': field_separator, 'download_filename': download_filename, 'csv_columns': csv_columns},
            type: 'POST',
            success: function(Response) {
                ...
                if(Response['data']['request_number'] <= Response['data']['total_requests']) {
                    ...
                    if(Response['status'] == true) {
                        setTimeout(function() {
                            ...
                            jQuery('body').append('<iframe id="ifrm_download" src="'+sn_wie_admin_url+'admin-post.php?action=sn_wie_download_product_category_export_file&file_name='+file_name+'&download_filename='+download_filename+'" style="display:none;"></iframe>');
    ...
    function import_csv_file(file_name, total_requests, request_number, import_columns, category_search_type, category_operation, field_separator) {

        jQuery.ajax({
            url: jQuery('.import-form').attr('action'),
            data: {'action': 'sn_wie_import_product_category_csv_file', 'file_name': file_name, 'total_requests': total_requests, 'request_number': request_number, 'import_columns': import_columns, 'category_search_type': category_search_type, 'category_operation': category_operation, 'field_separator': field_separator},
            type: 'POST',
            success: function(Response) {
                ...
                if(Response['data']['request_number'] < Response['data']['total_requests']) {
                    ...
                    import_csv_file(Response['data']['file_name'], Response['data']['total_requests'],Response['data']['request_number'], import_columns, category_search_type, category_operation, field_separator);
                }
                else {
                    setTimeout(function() {
                        update_import_status(Response['data']);
    ...
    function update_export_status(data) {
        ...
        export_count += data['product_category_export_count'];
    ...
    function update_import_status(data) {
        ...
        update_count += data['category_update_count'];
        insert_count += data['category_insert_count'];
        skip_count += data['category_skip_count'];
        fail_count += data['category_fail_count'];
        ...
        for(var p=0;p<data['category_status'].length;p++) {
            response += '<tr><td class="id">'+data['category_status'][p]['id']+'</td><td class="slug">'+data['category_status'][p]['slug']+'</td><td class="message">'+data['category_status'][p]['message']+'</td><td class="status">'+(data['category_status'][p]['status']==true?'<span class="success">Success</span>':'<span class="error">Error</span>')+'</td></tr>';
        }
